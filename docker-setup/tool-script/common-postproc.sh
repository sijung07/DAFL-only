...
cp -f /benchmark/bin/ASAN/$1/$2-$3 ./$2-$3
echo "Crash Replay log for $2-$3" > output/replay_log.txt
...
for crash in $CRASH_LIST; do
    ...
    if [[ $5 == "stdin" ]]; then
        cat output/crashes/$crash | timeout -k 30 15 ./$2-$3 $4 2>> output/replay_log.txt
    elif [[ $5 == "file" ]]; then
        cp -f output/crashes/$crash ./@@
        timeout -k 30 15 ./$2-$3 $4 2>> output/replay_log.txt
        echo "Exit value is $(echo $?)" >> output/replay_log.txt
    else
        echo "Invalid input source: $5"
        exit 1
    fi
done
...
if [[ "$2" == "nm" ]]; then
    ...
    echo -e "\n\nHang Replay log for $2-$3" >> output/replay_log.txt
